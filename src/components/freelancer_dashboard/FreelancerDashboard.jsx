...
const FreelancerDashboard = () => {
  ...
  const quickActions = [
    ...
    { icon: Timer, label: 'Track Time', color: 'bg-emerald-600 hover:bg-emerald-700' },
    { icon: CreditCard, label: 'Create Invoice', color: 'bg-purple-600 hover:bg-purple-700' },
    { icon: Mail, label: 'Send Proposal', color: 'bg-amber-600 hover:bg-amber-700' }
  ];
      ...
      <header className="bg-gray-800 border-b border-gray-700 px-8 py-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 mt-1">Welcome back! Here's your freelance overview.</p>
          </div>
      ...
      <main className="p-8 overflow-y-auto max-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {statsCards.map((stat, index) => (
            <div key={index} className="bg-gray-800 rounded-2xl p-6 border border-gray-700 hover:border-gray-600 transition-all duration-300 hover:shadow-xl">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {quickActions.map((action, index) => (
              <button
                key={index}
                className={`${action.color} rounded-xl p-4 flex flex-col items-center space-y-3 transition-all duration-200 transform hover:scale-105 hover:shadow-lg`}
              >
            ...
            <div className="p-6">
              <div className="space-y-4">
                {recentProjects.map((project) => (
                  <div key={project.id} className="bg-gray-700 rounded-xl p-4 hover:bg-gray-650 transition-colors duration-200">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="text-right">
                        <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${project.statusColor} bg-opacity-20`}>
                          {project.status}
                        </div>
                        <p className={`text-xs mt-1 ${project.priorityColor}`}>{project.priority} Priority</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-semibold text-emerald-400">{project.budget}</span>
                    </div>
                  </div>
                ))}
              </div>
